station = {'Anand Vihar Delhi';'IIT Chennai';'Maharastra Pollution Control Board Mumbai';'Manali Chennai';'Navi Mumbai Municipal Corp ';'Nehru Nagar Kanpur';'Punjabi Bagh Delhi';'R Bharati Unv Kolkata';'R K Puram Delhi';'Victoria Memorial Kolkata'};


for k = 1:10
file_name = strcat(station{k},' SO2.xlsx');
...
for m = 1:34
    ...
    if monthval(m) > min(data2(:,1)) && monthval(m) < max(data2(:,1))
        ...
        if m == 22
            t = text(monthval(m),y0(2) + 1,strcat(month(m,:),' (Oct)'),'HorizontalAlignment','left','FontSize',8);
        else
            t = text(monthval(m),y0(2) + 1,month(m,:),'HorizontalAlignment','left','FontSize',8);
        end
        set(t,'Rotation',90);
    end
    hold on
...
text(max(data2(:,1))+2,c(1),'Good','HorizontalAlignment','left','color',[0 0.6 0]);
p1.LineWidth = 1.2;
hold on
...
text(max(data2(:,1))+2,c(2),'Satisfactory','HorizontalAlignment','left','color',[0.3 0.6 0]);
p2.LineWidth = 1.2;
hold on
end
...
text(max(data2(:,1))+2,c(3),'Moderate','HorizontalAlignment','left','color',[0.6 0.6 0]);
p3.LineWidth = 1.2;
hold on
end
...
text(max(data2(:,1))+2,c(4),'Poor','HorizontalAlignment','left','color',[0.6 0.3 0]);
p4.LineWidth = 1.2;
hold on
end
...
text(max(data2(:,1))+2,c(5),'Very Poor','HorizontalAlignment','left','color',[0.6 0 0]);
p5.LineWidth = 1.2;
end
...
headin = strcat('Sulphur dioxide Concentration Trends in',{' '},station{k});
headin = headin + "\n\n\n\n\n";
...
hold off;
saveas(gcf,strcat(station{k},' SO2.jpeg'));
